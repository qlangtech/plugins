...
object HiveThriftServer2 extends Logging {
  ...
  private[thriftserver] class HiveThriftServer2Listener(
    ...
    def onStatementStart(
      ...
      val oplog = org.apache.hadoop.hive.ql.session.OperationLog.getCurrentOperationLog;
      if (oplog == null) {
        throw new IllegalStateException("operationLog can not be null");
      }
      val info = new ExecutionInfo(statement, sessionId, System.currentTimeMillis, userName, oplog)
    ...
    // baisui add
    def getOperationLog(statementId: String): OperationLog = {
      executionList.get(statementId) match {
        case Some(v) => v.operationLog;
        case None => throw new IllegalStateException(s"statementid:${statementId} can not find relevant OperationLog");
      }
    }
