...
public class FTPServer extends ParamsConfig {
    ...
    public FtpHelper createFtpHelper(Integer timeout, TDFSLinker dfsLinker) {
        return FtpHelper.createFtpClient(dfsLinker, this.protocol, this.host, this.username, this.password, this.port, timeout, this.connectPattern);
    }

    public <T> T useFtpHelper(TDFSSessionVisitor<T> helperConsumer, TDFSLinker dfsLinker) {
        try (FtpHelper ftpHelper = this.createFtpHelper(this.timeout, dfsLinker)) {
