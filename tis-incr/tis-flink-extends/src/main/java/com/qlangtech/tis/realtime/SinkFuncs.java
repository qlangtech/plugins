...
public final class SinkFuncs<TRANSFER_OBJ> {
    ...
    private transient final Map<IDataxProcessor.TableMap, TabSinkFunc<?, ?, TRANSFER_OBJ>> sinkFunction;
    // public transient final CountDownLatch countDown;

    public SinkFuncs(Map<IDataxProcessor.TableMap, TabSinkFunc<?, ?, TRANSFER_OBJ>> sinkFunction) {
    ...
    public void add2Sink(String originTableName, DTOStream sourceStream) {
        ...
        if (sinkFunction.size() < 2) {
            for (Map.Entry<IDataxProcessor.TableMap, TabSinkFunc<?, ?, TRANSFER_OBJ>> entry : sinkFunction.entrySet()) {
