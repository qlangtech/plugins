...
//                }
    ...
    public HudiTableMeta(Configuration paramCfg) {
        this(paramCfg, paramCfg.getNecessaryValue(Key.FILE_NAME, HdfsWriterErrorCode.REQUIRED_VALUE));
    }
    ...
    public IPath getDumpDir(ITISFileSystem fs, IHiveConnGetter hiveConn) {
        if (this.tabDumpDir == null) {
            this.tabDumpDir = getDumpDir(fs, hudiTabName, BasicHdfsWriterJob.getDumpTimeStamp(), hiveConn);// fs.getPath(fs.getRootDir(), hiveConn.getDbName() + "/" + dumpTimeStamp + "/" + this.hudiTabName);
        }
