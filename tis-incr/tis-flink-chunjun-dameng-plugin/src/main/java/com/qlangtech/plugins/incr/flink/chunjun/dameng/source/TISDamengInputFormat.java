...
import com.dtstack.chunjun.connector.dameng.source.DamengInputFormat;
 ...
 **/
public class TISDamengInputFormat extends DamengInputFormat {
    private final DataSourceFactory dataSourceFactory;
    private final TableCols tableCols;

    public TISDamengInputFormat(DataSourceFactory dataSourceFactory, List<IColMetaGetter> cols) {
    ...
    protected final void initializeRowConverter() {
        this.setRowConverter(DialectUtils.createColumnConverter(
                jdbcDialect, jdbcConf, this.colsMeta, TISDamengInputFormat::createInternalConverter));
    }
    ...
    private static IDeserializationConverter createInternalConverter(LogicalType type) {
        if (type.getTypeRoot() == LogicalTypeRoot.DATE) {
            return val -> new DamengDateColumn((Timestamp) val);
        }
        return DamengColumnConverter.createInternalConverter(type);
    }
