...
import com.twodfire.async.message.client.consumer.RocketMQListenerFactory;
import com.twodfire.async.message.client.to.impl.HessianDeserialize;
...
public class TestIncrComponent extends BaseTestCase {
    ...
    public void testLoad() {
        ...
        List<AbstractMQListenerFactory> mqListenerFactoryList = incrComponent.getMqListenerFactory();
        assertEquals(1, mqListenerFactoryList.size());
        AbstractMQListenerFactory mqListenerFactory = mqListenerFactoryList.stream().findFirst().get();
