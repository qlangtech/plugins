...
public class FTPContainer extends GenericContainer {
    ...
    public FTPContainer() {
        super(FTP_DOCKER_IMAGE_NAME);
        this.addExposedPorts(new int[]{PORT21, PORT20});
        for (int i = MIN_PASV_PORT; i <= MAX_PAV_PORT; i++) {
            this.addExposedPorts(i);
        }
        this.addEnv("FTP_USER", "test");
        this.addEnv("FTP_PASS", "test");
    ...
    public int getPort20() {
        return this.getMappedPort(PORT20);
    }
}
