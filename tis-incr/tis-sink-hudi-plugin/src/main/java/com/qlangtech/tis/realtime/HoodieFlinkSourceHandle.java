...
public abstract class HoodieFlinkSourceHandle extends BasicFlinkSourceHandle {
        ...
        try {
            for (Map.Entry<String, DTOStream> entry : tab2OutputTag.entrySet()) {
                this.registerTable(env, Objects.requireNonNull(tabStreamerCfg.get(entry.getKey())
                        , "tab:" + entry.getKey() + " relevant instance of " + FlinkStreamerConfig.class.getSimpleName() + " can not be null")
    ...
    protected abstract Map<String, FlinkStreamerConfig> createTabStreamerCfg();

    private void registerTable(StreamExecutionEnvironment env, FlinkStreamerConfig tabStreamerCfg
            , String tabName, DTOStream dtoDataStream) throws Exception {
        int parallelism = env.getParallelism();

        // HudiSinkFactory sinkFuncFactory = (HudiSinkFactory) getSinkFuncFactory();

        final FlinkStreamerConfig cfg = tabStreamerCfg;
